...
public class CloudsController {
    ...
    public void positionClouds(boolean firstTimeArranging) {
        ...
        for (Cloud c : clouds) {
            if (c.getX() == 0 && c.getY() == 0) {
                float tempX = 0;
                if (controlX == 0) {
                    tempX = randomBetweenTwoNumbers(maxX - 40, maxX);
                    controlX = 1;
                    c.setDrawLeft(false);
                } else if (controlX == 1) {
                    tempX = randomBetweenTwoNumbers(minX + 40, minX);
                    controlX = 0;
                    c.setDrawLeft(true);
                }
    ...
    public void drawClouds(SpriteBatch batch) {
        for (Cloud c : clouds) {
            if (c.getDrawLeft()) {
                batch.draw(c, c.getX() - c.getWidth() + 130 / 2, c.getY() - c.getHeight() / 2);
            } else {
                batch.draw(c, c.getX() - c.getWidth() + 200 / 2, c.getY() - c.getHeight() / 2);
            }
        }
    }
    ...
    private float randomBetweenTwoNumbers(float min, float max) {
    ...
    public void removeOffScreenCollectables(){
        for (int i = 0; i < collectables.size; i++) {
            if ((collectables.get(i).getY() - GameInfo.HEIGHT / 2 - 20) > cameraY) {
                collectables.get(i).getTexture().dispose();
                collectables.removeIndex(i);
            }
        }
